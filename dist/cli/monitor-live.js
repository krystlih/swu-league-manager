...
async function displayLiveDashboard() {
    const [activeLeagues, recentMatches, pendingMatches, recentRegistrations, stats] = await Promise.all([
        prismaClient_1.prisma.league.findMany({
        ...
        prismaClient_1.prisma.match.findMany({
            where: { isCompleted: true },
            include: { player1: true, player2: true, league: true, round: true },
            orderBy: { reportedAt: 'desc' },
            take: 10
        }),
        prismaClient_1.prisma.match.count({ where: { isCompleted: false } }),
        prismaClient_1.prisma.registration.findMany({
    ...
    if (activeLeagues.length === 0) {
        ...
        activeLeagues.forEach((league, idx) => {
            ...
            const completionBar = generateProgressBar(league._count.matches - pendingCount, league._count.matches, 20);
...
async function displayStatistics() {
    const [totalLeagues, activeLeagues, completedLeagues, totalPlayers, totalMatches, completedMatches, totalRegistrations] = await Promise.all([
        prismaClient_1.prisma.league.count(),
...
function startAutoRefresh(viewFunction) {
