...
exports.EliminationService = void 0;
class EliminationService {
     ...
     */
    static generateSingleEliminationBracket(players) {
     ...
     */
    static generateNextSingleEliminationRound(completedMatches, currentRoundNumber) {
     ...
     */
    static generateDoubleEliminationBracket(players) {
     ...
     */
    static generateNextDoubleEliminationRound(winnersMatches, losersMatches, currentRoundNumber) {
        const result = {
            winnersBracketPairings: [],
            losersBracketPairings: [],
            grandFinals: false,
        };
        // Generate winners bracket next round (same as single elimination)
        if (winnersMatches.length > 1) {
            result.winnersBracketPairings = this.generateNextSingleEliminationRound(winnersMatches.map(m => ({ winnerId: m.winnerId, winnerName: m.winnerName, matchNumber: m.matchNumber })), currentRoundNumber);
        }
     ...
     */
    static generateGrandFinals(winnersChampion, losersChampion) {
        return [{
                tableNumber: 1,
     ...
     */
    static needsBracketReset(winnersChampionId, grandFinalsWinnerId) {
     ...
     */
    static calculateSingleEliminationRounds(numPlayers) {
        if (!this.isPowerOfTwo(numPlayers)) {
     ...
     */
    static calculateDoubleEliminationRounds(numPlayers) {
        if (!this.isPowerOfTwo(numPlayers)) {
            throw new Error('Number of players must be a power of 2');
        }
     ...
     */
    static getSingleEliminationRoundName(roundNumber, totalRounds) {
        ...
        if (roundNumber === 1 && totalRounds > 4)
     ...
     * @returns Human-readable round name
     */
    static getDoubleEliminationRoundName(roundNumber, isLosersBracket, isGrandFinals) {
        if (isGrandFinals)
            return 'Grand Finals';
        if (isLosersBracket)
     ...
     */
    static getPreviousPowerOfTwo(n) {
        return Math.pow(2, Math.floor(Math.log2(n)));
    }
}
exports.EliminationService = EliminationService;
